...
import { subscribeEventos, eliminarEvento, toggleAsistenciaEvento } from "../services/eventos";
...
export const EventsScreen: React.FC = () => {
  ...
  const [filter, setFilter] = useState<"todos" | "encuestas" | EventoCategoria>("todos");
  ...
  const combinedItems = React.useMemo(() => {
    if (filter === "encuestas") {
      return encuestas.map((e) => ({ type: "encuesta" as const, data: e, date: e.fechaCreacion || "" }));
    }
    if (filter === "todos") {
      const evs = eventos.map((e) => ({ type: "evento" as const, data: e, date: e.fechaActualizacion || e.fechaCreacion || "" }));
      const encs = encuestas.map((e) => ({ type: "encuesta" as const, data: e, date: e.fechaCreacion || "" }));
      return [...evs, ...encs].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    }
    return eventos
      .filter((e) => e.categoria === filter)
      .map((e) => ({ type: "evento" as const, data: e, date: e.fechaActualizacion || e.fechaCreacion || "" }));
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>Citaciones, asambleas, cortes y encuestas comunitarias</Text>
      </View>
      ...
      <View style={styles.filterRow}>
        {[
          ...
          >
            <Text style={[styles.filterChipText, filter === f.value && styles.filterChipTextSelected]}>
              {f.label}
            </Text>
          </TouchableOpacity>
        ))}
        ...
        <FlatList
            ...
            if (item.type === "evento") {
              ...
              return (
                <EventCard
                  ...
                  onToggleLike={async () => {
                    if (!user) return;
                    await toggleLikeEntidad(e.id, "eventos", user.uid, (e as any).likes || []);
                  }}
              ...
              const totalVotos = (p.opcionesEncuesta || []).reduce((acc, curr) => acc + (curr.votos?.length || 0), 0);

              return (
                <View style={{ backgroundColor: "#FFFFFF", borderRadius: theme.borderRadius.lg, padding: theme.spacing.md, marginBottom: theme.spacing.md, borderWidth: 1, borderColor: "#E5E7EB" }}>
                  ...
                  <Text style={{ fontSize: 16, fontWeight: "bold", color: "#1A1D1A", marginVertical: 6 }}>
          ...
          ListEmptyComponent={
            <View style={styles.emptyBox}>
              ...
              <Text style={styles.emptyTitle}>No hay elementos en esta sección</Text>
              <Text style={styles.emptySubtitle}>Los eventos y encuestas oficiales aparecerán aquí.</Text>
            </View>
          }
        />
      )}
...
const styles = StyleSheet.create({
  ...
  header: { padding: theme.spacing.lg, paddingTop: theme.spacing.xl, backgroundColor: "#FFFFFF", borderBottomWidth: 1, borderBottomColor: "#E2E8F0" },
  title: { fontSize: 22, fontWeight: "bold", color: "#234919" },
  subtitle: { fontSize: 13, color: "#64748B", marginTop: 2 },
  filterRow: { flexDirection: "row", paddingHorizontal: theme.spacing.lg, paddingVertical: theme.spacing.md, gap: 6, flexWrap: "wrap" },
  ...
  fab: { position: "absolute", bottom: 24, right: 24, width: 56, height: 56, borderRadius: 28, backgroundColor: "#234919", alignItems: "center", justifyContent: "center", elevation: 4 },
  emptyBox: { alignItems: "center", paddingVertical: theme.spacing.xl },
  emptyTitle: { fontSize: 16, fontWeight: "bold", color: "#0F172A", marginTop: 8 },
  ...
  modalContent: { backgroundColor: "#FFFFFF", borderRadius: theme.borderRadius.lg, padding: theme.spacing.lg, maxHeight: "80%" },
  modalTitle: { fontSize: 18, fontWeight: "bold", color: "#234919" },
  modalSubtitle: { fontSize: 13, color: "#64748B", marginTop: 2, marginBottom: 8 },
  badgeCountContainer: { backgroundColor: "#F1F5F9", borderRadius: 8, paddingHorizontal: 10, paddingVertical: 6, marginBottom: 12, alignSelf: "flex-start" },
  attendeesTotalText: { fontSize: 12, fontWeight: "600", color: "#234919" },

  modalLoadingBox: { paddingVertical: 24, alignItems: "center", justifyContent: "center" },
  modalLoadingText: { marginTop: 8, fontSize: 13, color: "#64748B" },

  modalEmptyBox: { paddingVertical: 24, alignItems: "center", justifyContent: "center" },
  modalEmptyText: { marginTop: 8, fontSize: 13, color: "#64748B", textAlign: "center" },

  attendeesList: { maxHeight: 280, marginVertical: 4 },
  attendeeCard: { flexDirection: "row", alignItems: "center", backgroundColor: "#F8FAFC", borderRadius: theme.borderRadius.md, padding: 10, marginBottom: 8, borderWidth: 1, borderColor: "#E2E8F0" },
  attendeeIconBox: { width: 32, height: 32, borderRadius: 16, backgroundColor: "#DCFCE7", alignItems: "center", justifyContent: "center", marginRight: 10 },
  attendeeInfo: { flex: 1 },
  attendeeName: { fontSize: 14, fontWeight: "600", color: "#0F172A" },
  attendeeInmueble: { fontSize: 12, color: "#64748B", marginTop: 2 },
  roleBadge: { backgroundColor: "#E2E8F0", paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6 },
  roleBadgeText: { fontSize: 10, fontWeight: "bold", color: "#334155", textTransform: "capitalize" },

  closeBtn: { height: 44, backgroundColor: "#234919", borderRadius: theme.borderRadius.md, alignItems: "center", justifyContent: "center", marginTop: 12 },
